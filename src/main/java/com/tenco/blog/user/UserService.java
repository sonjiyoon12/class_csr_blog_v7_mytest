...
public class UserService {
    ...
    public UserResponse.UpdateDTO updateById(Long userId, UserRequest.UpdateDTO updateDTO){
        ...
        // 1. 권한 체크
        User seletedUser = userJpaRepository.findById(userId)
                        .orElseThrow(() -> {
                            throw new Exception404("사용자를 찾을 수 없습니다");
                        });
        // 2. 더티 체킹을 통한 회원 정보 수정
        seletedUser.update(updateDTO);

        // 3. 응답 DTO 반환
        return new UserResponse.UpdateDTO(seletedUser);
    }
}
